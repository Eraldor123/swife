...
import {
    ...
    type SelectChangeEvent, Dialog, DialogTitle, DialogContent, DialogActions // <--- PŘIDÁNO PRO TYPOVÁNÍ
    // <--- PŘIDÁNO PRO TYPOVÁNÍ
} from '@mui/material';
import { ArrowBack, ArrowBackIos, ArrowForwardIos } from '@mui/icons-material';
import { useNavigate } from 'react-router-dom';
import { ScheduleService } from '../../services/ScheduleService';
import type { WeeklyScheduleResponse, PlannerUser, HierarchyData } from '../../types/schedule';
...
export const ShiftPlanner = () => {
    ...
    const handleCopyConfirm = async (data: CopyFormValues) => {
        try {
            ...
            // Po úspěšném kopírování se rovnou přesuneme do toho nového týdne!
    ...
    // OPRAVA: Správně typovaný handler pro Select bez použití 'any'
    ...
    const handleClearConfirm = async () => {
        try {
            ...
            await loadData(); // Okamžitě překreslíme čistou mřížku
        } catch (error) {
            console.error("Chyba při mazání týdne:", error);
            alert("Nepodařilo se vyčistit týden. Zkontrolujte připojení k serveru.");
        }
    };
    ...
    return (
        <Box sx={{ display: 'flex', height: '100vh', backgroundColor: '#f2ece4', overflow: 'hidden' }}>
            <PlannerSidebar users={availableUsers} />
                ...
                <Paper elevation={0} sx={{
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                        <Button
                            ...
                            size="small"
                            variant="contained"
                            sx={{ bgcolor: '#d3d3d3', color: 'black', borderRadius: 2, fontWeight: 'bold', textTransform: 'none' }}
                        >
                            Generování
                        </Button>
                        <Button
                            onClick={() => setIsCopyModalOpen(true)}
                            size="small" variant="contained" sx={{ bgcolor: '#d3d3d3', color: 'black', borderRadius: 2, fontWeight: 'bold', textTransform: 'none' }}
                        >
                            Kopírování
                        </Button>
                        <Button
                            onClick={() => setIsClearModalOpen(true)}
                            size="small" variant="contained" color="error" sx={{ borderRadius: 2, fontWeight: 'bold', textTransform: 'none' }}
                        >
                            Vyčistit
                        </Button>
                    </Box>
                    ...
                    <FormControl size="small" sx={{ minWidth: 140 }}>
                        ...
                        <Select
                            ...
                            label="Hlavní typ"
                            onChange={handleCategoryChange} // <--- OPRAVENO
